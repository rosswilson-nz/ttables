extract_table <- function(x) {
  ...
  table <- combine_table_sections(header, body, added_rows)
  footnotes <- extract_footnotes(footnotes, table, direction = x$`_opts`$table.footnotes.direction)
...
extract_contents <- function(df, formats, layout, footnotes, location) {
  ...
  out <- lapply(seq_len(nrow(df)), \(i) lapply(df[i, ], \(j) as.list(j)))
  f_table <- dplyr::filter(formats, location == "table")
  for (r in seq_len(nrow(f_table))) {
    f <- as.list(f_table[r, ])
    rows <- seq_along(out)
    columns <- seq_along(df)
  ...
  f_header <- dplyr::filter(formats, location == !!location)
  for (r in seq_len(nrow(f_header))) {
    f <- as.list(f_header[r, ])
  ...
  l <- dplyr::filter(layout, location == !!location)
  ...
  tibble::as_tibble(purrr::list_transpose(out, simplify = FALSE))
}

combine_table_sections <- function(header, body, added_rows) {
  added_rows <- dplyr::arrange(added_rows, dplyr::desc(`_insert_before`))
  for (r in seq_len(nrow(added_rows))) {
    body <- tibble::add_row(body, dplyr::select(added_rows[r, ], -`_insert_before`), .before = added_rows$`_insert_before`[[r]])
  }
  dplyr::bind_rows(header, body)
}

extract_footnotes <- function(footnotes, table, direction) {
  footnote_num <- integer()
  footnote_alph <- integer()
  footnote_sym <- integer()
  if (direction == "horizontal") {
    for (r in seq_len(nrow(table))) for (c in seq_len(ncol(table))) {
      footnote_num <- append(footnote_num, attr(table[[c]][[r]], "footnote_num"))
      footnote_alph <- append(footnote_alph, attr(table[[c]][[r]], "footnote_alph"))
      footnote_sym <- append(footnote_sym, attr(table[[c]][[r]], "footnote_sym"))
    }
  } else {
    for (c in seq_len(ncol(table))) for (r in seq_len(nrow(table))) {
      footnote_num <- append(footnote_num, attr(table[[c]][[r]], "footnote_num"))
      footnote_alph <- append(footnote_alph, attr(table[[c]][[r]], "footnote_alph"))
      footnote_sym <- append(footnote_sym, attr(table[[c]][[r]], "footnote_sym"))
    }
