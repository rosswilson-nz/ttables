new_table_opts <- function(
  ...
  if (missing(footnotes.order)) {
    footnotes.order <- c("general", "number", "alphabet", "symbol")
  }
  if (missing(footnotes.number)) {
    footnotes.number <- "arabic"
  }
  if (missing(footnotes.alphabet)) {
    footnotes.alphabet <- "lower"
  }
  if (missing(footnotes.symbol)) {
    footnotes.symbol <- "extended"
  }
  if (missing(footnotes.direction)) {
    footnotes.direction <- "horizontal"
  }
  if (missing(footnotes.size)) {
    footnotes.size <- "0.9em"
  }
  if (missing(supplement)) {
    supplement <- FALSE
  }
...
check_gutter <- function(x) {
  ...
  if (rlang::is_scalar_character(x)) {
    if (x == "auto") {
      ...
      n <- nchar(x)
      x <- if (substr(x, n - 1, n) == "fr") {
        as_fractional_length(x)
      } else {
        as_relative(x)
      }
    }
  ...
  if (
    rlang::is_bare_list(x) &&
      rlang::is_named(x) &&
      all(names(x) %in% c("row", "column"))
  ) {
    if (length(x$column) > 1 || length(x$row) > 1) {
      rlang::abort("Row and column gutters must be scalar lengths")
    }
    n <- nchar(x$column)
...
check_caption <- function(x) {
  if (is.null(x) || rlang::is_scalar_character(x)) {
    return(x)
  }
...
check_footnotes.order <- function(x) {
  if (
    rlang::is_character(x, 4) &&
      setequal(x, c("general", "number", "alphabet", "symbol"))
  ) {
    return(x)
  }
  rlang::abort(
    "'footnotes.order' must be a permutation of c('general', 'number', 'alphabet', 'symbol')"
  )
}

check_footnotes.number <- function(x) {
  if (rlang::is_scalar_character(x) && x %in% c("arabic", "roman", "Roman")) {
    return(x)
  }
  rlang::abort(
    "'footnotes.number' must be one of c('arabic', 'roman', 'Roman')"
  )
}

check_footnotes.alphabet <- function(x) {
  if (rlang::is_scalar_character(x) && x %in% c("lower", "upper")) {
    return(x)
  }
...
check_footnotes.symbol <- function(x) {
  if (rlang::is_scalar_character(x) && x %in% c("standard", "extended")) {
    return(x)
  }
  ...
  rlang::abort(
    "'footnotes.symbol' must be one of c('standard', 'extended') to use the predefined sets, or a character vector of symbols"
  )
}

check_footnotes.direction <- function(x) {
  if (rlang::is_scalar_character(x) && x %in% c("horizontal", "vertical")) {
    return(x)
  }
  rlang::abort(
    "'footnotes.direction' must be one of c('horizontal', 'vertical')"
  )
}

check_footnotes.size <- function(x) {
  if (rlang::is_scalar_character(x)) {
    return(as_length(x))
  }
  rlang::abort(
    "'footnotes.size' must be a scalar length specification"
  )
}
...
#'     `"horizontal"`.
#' @param footnotes.size The fontsize for footnotes. Should be a valid Typst
#'     length specification. Default is `"0.9em"`.
...
set_table_options <- function(
  ...
  if (!missing(align)) {
    opts$align <- check_align(align, ncol(x$`_data`))
  }
  if (!missing(widths)) {
    opts$widths <- check_widths(widths, ncol(x$`_data`))
  }
  if (!missing(placement)) {
    opts$placement <- check_placement(placement)
  }
  if (!missing(gutter)) {
    opts$gutter <- check_gutter(gutter)
  }
  if (!missing(caption)) {
    opts$caption <- check_caption(caption)
  }
  if (!missing(label)) {
    opts$label <- check_label(label)
  }
  if (!missing(footnotes.order)) {
    opts$footnotes.order <- check_footnotes.order(footnotes.order)
  }
  if (!missing(footnotes.number)) {
    opts$footnotes.number <- check_footnotes.number(footnotes.number)
  }
  if (!missing(footnotes.alphabet)) {
    opts$footnotes.alphabet <- check_footnotes.alphabet(footnotes.alphabet)
  }
  if (!missing(footnotes.symbol)) {
    opts$footnotes.symbol <- check_footnotes.symbol(footnotes.symbol)
  }
  if (!missing(footnotes.direction)) {
    opts$footnotes.direction <- check_footnotes.direction(footnotes.direction)
  }
  if (!missing(footnotes.size)) {
    opts$footnotes.size <- check_footnotes.size(footnotes.size)
  }
  if (!missing(supplement)) {
    opts$supplement <- check_supplement(supplement)
  }
  if (!missing(na)) {
    opts$na <- check_na(na)
  }
  if (!missing(landscape)) {
    opts$landscape <- check_landscape(landscape)
  }
